...
import { openWhiteboardDialog } from '../stores/PersonalStore'

export default class Personal extends Item {
  id?: string
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, texture: string, frame?: string | number) {
    ...
    this.itemType = ItemType.PERSONAL
  }
  ...
  onOverlapDialog() {
    if (this.currentUsers.size === 0) {
      this.setDialogBox('Press R to use 개인방')
    } else {
      this.setDialogBox('Press R to use 개인방')
    }
  }
